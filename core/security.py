from datetime import datetime, timedelta
from typing import Any, Union

from jose import jwt, JWTError
from passlib.context import CryptContext

import schemas
import core.configuration as conf


pwd_context = CryptContext(schemes=["bcrypt"], deprecated="auto")


def create_access_token(
    subject: Union[str, Any], expires_delta: timedelta = None, secret=conf.SECRET_KEY
) -> str:
    if expires_delta:
        expire = datetime.utcnow() + expires_delta
    else:
        expire = datetime.utcnow() + timedelta(minutes=conf.ACCESS_TOKEN_EXPIRE_MINUTES)
    to_encode = {"exp": expire, "sub": str(subject)}
    encoded_jwt = jwt.encode(to_encode, secret, algorithm=conf.ALGORITHM)
    return encoded_jwt


def verify_access_token(token: str, secret=conf.SECRET_KEY) -> schemas.TokenData | None:
    """Верифицирует, в случае, если всё окей, возвращает token_data, иначе None"""
    if not token:
        return None

    try:
        payload = jwt.decode(token, secret, algorithms=[conf.ALGORITHM])
        if (email := payload.get("sub")) is None:
            return None
        token_data = schemas.TokenData(email=email)
    except JWTError:
        return None
    return token_data


def verify_password(plain_password: str, hashed_password: str) -> bool:
    return pwd_context.verify(plain_password, hashed_password)


def get_password_hash(password: str) -> str:
    return pwd_context.hash(password)
