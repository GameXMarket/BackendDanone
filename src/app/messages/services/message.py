...
class BaseChatManager:
    ...
    async def get_chat_by_user_id(
        self, db_session: AsyncSession, user_id: int, author_id: int
    ) -> int | None:
        """
        ...
        stmt = (
            ...
            .where(models_m.ChatMember.chat_id == models_m.Chat.id)
            .where(models_m.ChatMember.user_id.in_([user_id, author_id]))
        )
        result = await db_session.execute(stmt)
        return result.scalar_one_or_none()
...
class BaseChatMemberManager(BaseChatManager):
    ...
    async def get_chat_member(
        db_session: AsyncSession, member_id: int
    ...
    async def update_chat_member(
        db_session: AsyncSession, member_id: int, new_user_id: int, new_chat_id: int
    ) -> None:
        pass
...
class BaseMessageManager(BaseChatManager):
    def __init__(self) -> None:
        super().__init__()

    async def create_message(
        db_session: AsyncSession, chat_member_id: int, content: str, created_at: int
    ) -> models_m.Message:
    ...
    async def update_message(
        db_session: AsyncSession, message_id: int, new_content: str, new_created_at: int
    ) -> None:
        pass
...
class ChatConnectionManager:
    ...
    async def start_listening(self, conn_context: ConnectionContext):
        while True:
            try:
                new_message: schemas_m.MessageCreate = schemas_m.MessageCreate.model_validate(
